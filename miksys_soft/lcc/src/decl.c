...
#define bits2bytes(n) (((n) + byte_size-1)/byte_size)
static int regcount;
...
static void fields(Type ty) {
	  ...
	  while (istypename(t, tsym)) {
	  	...
	  	for (;;) {
			...
			if (t == ':') {
				...
				if (p->bitsize > byte_size*inttype->size || p->bitsize < 0) {
					error("`%d' is an illegal bit-field size\n",
						p->bitsize);
					p->bitsize = byte_size*inttype->size;
	...
	{ int bits = 0, off = 0, overflow = 0;
	  ...
	  for (p = *q; p; p = p->link) {
		...
		|| bits - 1 + p->bitsize > byte_size*unsignedtype->size) {
		...
		if (p->lsb) {
			...
			if (IR->little_endian)
				...
				p->lsb = byte_size*unsignedtype->size - bits + 1
					- p->bitsize + 1;
			bits += p->bitsize;
		} else
