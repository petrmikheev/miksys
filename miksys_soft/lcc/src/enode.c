...
Tree call(Tree f, Type fty, Coordinate src) {
	...
	Type *proto, *proto_end, rty = unqual(freturn(fty));
	Symbol t3 = NULL;
	...
	if (t != ')') {
		...
		int arg_n;
		for (;;) {
			args_buf[args_count++] = pointer(expr1(0));
			if (t != ',')
				break;
			t = gettok();
		}
		if (IR == &miksysIR) {
			...
			for (j=0;j<args_count;++j) {
				if (proto && *proto && *proto != voidtype) proto++;
			}
			proto_end = proto;
		}
		for (arg_n=0; arg_n<args_count; ++arg_n) {
			Tree q = args_buf[IR == &miksysIR ? args_count-arg_n-1 : arg_n]; //pointer(expr1(0));
			if (IR == &miksysIR && proto && fty->u.f.proto + args_count-1-arg_n<proto) proto = fty->u.f.proto + args_count-1-arg_n;
	...
	expect(')');
	if (IR == &miksysIR) proto = proto_end;
	if (proto && *proto && *proto != voidtype)
...
Tree cnsttree(Type ty, ...) {
	...
	case UNSIGNED:p->u.v.u = va_arg(ap, unsigned long)&ones(byte_size*ty->size); break;
...
Tree asgntree(int op, Tree l, Tree r) {
	...
	if (l->op == FIELD) {
		long n = byte_size*l->u.field->type->size - fieldsize(l->u.field);
