...
-   С целью увеличения производительности были приняты некоторые
    ...
    старого места. [Ниже](#некоторые-особенности) приведен список подобных
...
-   [Регистры](#регистры)
-   [Суффиксы условного выполнения](#суффиксы-условного-выполнения)
...
0x01   | [J](#j-call-ret)      | Переход по адресу (без сброса конвеера)
0x01   | [CALL](#j-call-ret)   | Вызов функции (без сброса конвеера)
0x01   | [RET](#j-call-ret)    | Возврат из функции (также используется для перехода по абсолютному адресу; без сброса конвеера)
0x02   | [CMOV](#cmov) | Запись константы в регистр или спец. регистр
0x03   | [CCMOV](#ccmov) | Запись константы в кэш
0x05   | [DJ](#dj-dcall-dret)    | Переход по адресу со сбросом конвеера
0x05   | [DCALL](#dj-dcall-dret) | Вызов функции со сбросом конвеера
0x05   | [DRET](#dj-dcall-dret)  | Возврат из функции со сбросом конвеера (также используется для перехода по абсолютному адресу)
0x0C   | [SHL](#shl-shr-mul-smul)  | Битовый сдвиг влево. Результат сохраняется в спец. регистр.
0x0C   | [SHR](#shl-shr-mul-smul)  | Битовый сдвиг в право. Результат сохраняется в спец. регистр.
0x0D   | [MUL](#shl-shr-mul-smul)  | Беззнаковое умножение. Результат сохраняется в спец. регистры.
0x0D   | [WMUL](#wmul) | Двойное умножение (32bit x 16bit) с отбрасыванием младших 16bit. Результат сохраняется в спец. регистры.
0x0E   | [SMUL](#shl-shr-mul-smul) | Умножение со знаком (доступно только в PU0). Результат сохраняется в спец. регистры.
0x0F   | [READ](#read-write)  | Чтение блока данных из оперативной памяти в кэш
0x0F   | [WRITE](#read-write) | Запись блока данных из кэша в оперативную память
0x10   | [OR](#арифметические-команды) | Побитовое ИЛИ
0x11   | [XOR](#арифметические-команды) | Побитовое исключающее ИЛИ
0x12   | [AND](#арифметические-команды) | Побитовое И
0x13   | [TST](#арифметические-команды)  | Побитовое И без сохранения результата (меняются толко флаги)
0x14   | [MOV](#mov)  | Копирование значения из регистра в регистр или между регистром и кэшем
0x14   | [IN](#in-out)    | Чтение из порта ввода-вывода. Не может быть условным.
0x14   | [OUT](#in-out)   | Запись в порт ввода-вывод. Не может быть условным.
0x15   | [LPU0-LPU3](#lpu0-lpu3) | Загрузка из кэша в конкретный PU (не реализовано)
0x16   | [RGBADD](#арифметические-команды) | Сложение двух цветов в формате R5G6B5
0x17   | [RGBSHR](#арифметические-команды) | Сдвиг на 1 бит вправо цвета в формате R5G6B5 (яркость уменьшается в 2 раза). Двухадресная команда.
0x18   | [ADD](#арифметические-команды) | Сложение
0x19   | [RSB](#арифметические-команды) | Обратное вычитание (меняется порядок операндов)
0x1A   | [SUB](#арифметические-команды) | Вычитание
0x1B   | [CMP](#cmp)  | Сравнение (вычитание без сохранения результата)
0x1C   | [ADС](#арифметические-команды) | Сложение с учетом переноса в предыдущей команде
0x1D   | [RSС](#арифметические-команды) | Обратное вычитание с учетом переноса в предыдущей команде
0x1E   | [SBС](#арифметические-команды) | Вычитание с учетом переноса в предыдущей команде
